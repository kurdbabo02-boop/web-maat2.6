...
const PortfolioPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F3F2EF] text-[#1a2235]">
      <Header />
      <main className="pt-20">
          ...
          <div className="max-w-5xl mx-auto">
            ...
            >
              ...
              <h1 className="text-3xl md:text-[44px] font-bold text-white tracking-tight leading-tight mb-3">
                {language === 'nl' ? 'Gerealiseerde Werk' : 'Realized Work'}
              </h1>
              ...
              {projects.map((project, index) => (
                ...
                >
                  <Link to={`/portfolio/${project.id}`} className="block">
                    {/* Image */}
                    <div className="relative w-full overflow-hidden rounded-xl bg-slate-200" style={{ aspectRatio: '3/2' }}>
                      <img
                        src={project.image}
                        alt={project.title}
                        className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-[1.04]"
                      />
                      <div className="absolute inset-0 bg-[#1a2235]/0 group-hover:bg-[#1a2235]/15 transition-colors duration-500 rounded-xl" />

                      {/* Category */}
                      <div className="absolute top-3.5 left-3.5">
                        <span className="px-3 py-1.5 bg-white/88 backdrop-blur-sm text-[#1a2235] text-[10px] font-bold uppercase tracking-widest rounded-full">
        ...
        {/* Stats */}
        <section className="py-10 px-6 md:px-16 border-t border-slate-200/70 bg-[#F3F2EF]">
          <div className="max-w-5xl mx-auto">
